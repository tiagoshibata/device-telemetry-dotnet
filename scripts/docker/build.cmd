...
:: Build the container image
    ...
    if %BUILD_LATEST% == "no" (
        docker build --tag %DOCKER_IMAGE%:%APP_VERSION% --squash --compress --label "Tags=Azure,IoT,solutions,Telemetry,.NET" .
    ) else (
        docker build --tag %DOCKER_IMAGE%:%APP_VERSION% --tag %DOCKER_IMAGE%:latest --squash --compress --label "Tags=Azure,IoT,solutions,Telemetry,.NET" .
    )
